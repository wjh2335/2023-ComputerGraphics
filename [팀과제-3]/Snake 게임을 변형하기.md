...
## 2.1. 최적화하기
```
1) 시작할 때는 1점인데 죽고 재시작 될 때 0점으로 시작되는 오류 수정하기
2) 180도 회전이 되버려서 바로 죽어버리는 오류 수정하기
```

## 2.2. 편의성 추가
```
1) 사망 시 게임 종료 화면이 나오게 하기
2) 게임 종료 화면에서 스페이스 바를 누르면 재시작 되게 하기
```
...
```
1) [팀과제-1]에서 제작한 토끼 캐릭터를 바탕으로 뱀 게임을 수정한다.
2) 배경, 먹이, 토끼 등 그래픽을 이미지로 변경한다.
3) 먹이의 색깔을 랜덤으로 할당하고 해당 먹이를 먹게 되면 토끼는 먹은 먹이에 해당되는 색깔로 변경된다.
```
...
또한 p5.js 라이브러리를 이용하여 캔버스에 그래픽을 그리고, 이미지를 로드하고, 랜덤한 색상을 생성하는 등의 작업을 수행합니다.
```
